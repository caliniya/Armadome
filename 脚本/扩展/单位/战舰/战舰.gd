...
func 加载武器() -> void:
	for 序列 in 武器:
		...
		武器缓存.初始化武器(自身阵营 ,self ,武器[序列]["弹药"] , 武器[序列]["电容"] , int(序列))
		本体.add_child(武器缓存)
...
func _ready() -> void:
	...
	扫描范围节点 = Area2D.new()
	扫描范围节点.monitorable = false
	add_child(扫描范围节点)
	扫描范围形状 = CollisionShape2D.new()
	扫描范围形状.shape = CircleShape2D.new()
	扫描范围形状.shape.radius = 扫描范围
	扫描范围节点.add_child(扫描范围形状)
	...
	单位.载重更新(self)
	加载武器()
	扫描范围节点.area_entered.connect(雷达扫描)

func _process(delta: float) -> void:
	向目标移动(delta)

func 向目标移动(帧差值 : float) -> void:
	移动参数 = 导航.计算移动参数(self , 移动目标坐标 , 帧差值)
	实际速度 = 移动参数["速度"]
	global_position = 移动参数["位置"]
	本体.rotation = 移动参数["旋转"]
...
func 雷达扫描(扫描单位 : Area2D) -> void:
	if 单位.雷达扫描目标阵营检查(扫描单位 , self):
		雷达扫描敌人列表.append(扫描单位)
